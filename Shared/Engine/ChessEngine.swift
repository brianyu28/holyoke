...
class ChessEngine: ObservableObject {
    ...
    func handlePossibleLine(line: String) {
        ...
        let multipvLabelIndices = components.allIndices(of: "multipv")
        
        for multipvLabelIndex in multipvLabelIndices {
            let lineNumberStr = components[optional: multipvLabelIndex + 1]
            ...
            // Get centipawn score
            let subcomponents = Array(components[multipvLabelIndex...])
            ...
            let pvFirstMove = pvLabelIndex != nil ? subcomponents[optional: pvLabelIndex! + 1] : nil
            guard let pvFirstMove = pvFirstMove else {
                return
            }
            
            self.lines[lineIndex] = "\(cp) \(pvFirstMove)"
        }
    }
